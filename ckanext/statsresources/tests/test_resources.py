...
class TestStatsResources(FunctionalTestBase):
    ...
    def setup(self):
        ...
        class Options:
            config = filter(
                lambda x: x.startswith("--with-pylons"), sys.argv
            ).pop()[14:]
    ...
    def _upload_report(self, data, files):
        ...
        app.post(
            ...
            params=data,
            headers={"Authorization": str(s["apikey"]),},
    ...
    def test_strict_access_for_admin(self):
        ...
        nt.assert_equal(
            app.get(
                self.main_page, headers={"Authorization": str(s["apikey"])}
            ).status_int,
            200,
        )
        nt.assert_equal(
            app.get(
                self.dc_page, headers={"Authorization": str(s["apikey"])}
            ).status_int,
            200,
        )
