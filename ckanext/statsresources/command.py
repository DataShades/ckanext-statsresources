...
class StatsresourcesCommand(tk.CkanCommand):
    ...
    def command(self):
        ...
        self.__reports = [
            Report._make(x.split(":"))
            for x in config.get("statsresources.report_map", "").splitlines()
            if x.count(":") == 3
        ]
        self.sysadmin = (
        ...
        self.headers = {"X-CKAN-API-Key": "{0}".format(self.sysadmin.apikey)}
    ...
    def _generate(self, report_list=None):
        ...
        for report in reports:
            ...
            if not pkg:
                print(
                    "Unable to generate {0}: Dataset does not exists".format(
                        report
                    )
                )
            ...
            if resource and not resource.is_active():
                ...
                print(
                    "ERROR: Resource <{0.id}> in [{0.state}] state. Unable to generate {1}".format(
                        resource, report
                    )
                )
            if not resource:
            ...
            try:
                ...
                if op:
                    kwargs = dict([l.split(":") for l in op.split()])
            except Exception:
                pass
            ...
            self._upload_report(
                {"id": res_id, "name": report.title}, [("upload", get_file)]
            )
    ...
    def _get_report_content(self, report, kwargs):
        ...
        if report.format not in resp.headers["content-type"]:
            print(
                "{0.name}: Content-type from {1} does not contains report format.".format(
                    report, url
                )
            )
        return resp.content
    ...
    def _get_report_url(self, name, format, kwargs):
        return h.url_for(
            "report", report_name=name, qualified=True, format=format, **kwargs
        )
